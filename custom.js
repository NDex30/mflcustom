$(function () {
  ...
  if (qs("O") === "121") {
    var nflSpreadsByTeamID = getNFLSpreads(newWeek);
    wrapperTable = $("table caption span")
      .filter(function () {
    ...
    wrapperTable.find("tr td").each(function () {
      ...
      isPositive = isPositiveInteger(nflSpreadsByTeamID[teamID])
        ? " positive"
        : "";
      $(this)
        .find("label")
    ...
    success: function (data) {
      for (x in data.nflSchedule.matchup) {
        for (y in data.nflSchedule.matchup[x].team) {
          nflSpreads[data.nflSchedule.matchup[x].team[y].id] =
            data.nflSchedule.matchup[x].team[y].spread;
        }
      }
    },
  }).fail(function () {
